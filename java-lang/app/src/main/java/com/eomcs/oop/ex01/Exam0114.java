...
public class Exam0114 {
  ...
  // => 팩토리 메서드
  static Score createScore(String name, int kor, int eng, int math) { // 파라미터 = 로컬 변수
    Score s = new Score(); // 메서드 내에 레퍼런스 = 로컬 변수  /   new 로 시작하면 무조건 heap 메모리에 올라간다.
    ...
    // => 삭제되지 않습니다. 인스턴스는 '''힙(heap) 메모리에 생성'''됩니다.
