...
public class Exam0130 {
  ...
  public static void main(String[] args) {
    ...
    // 클래스 변수 사용법
    // 클래스명.스태틱변수명 = 값;
    // 클래스를 ''사용하는 순간'' 클래스가 로딩되고, 스태틱 변수는 ''자동 생성''된다.
    A.v1 = 100;
    A.v2 = true;
...
//   JVM이 관리하는 메모리로 로딩''하는 것.
// - ''클래스의 코드를 사용하는 시점''에 메모리(Method Area 영역)에 로딩된다.
...
//     예) new A();
// - ''한 번 클래스가 로딩되면 JVM을 종료할 때까지 유지''한다.
...
// 3) Method Area
//    - JVM이 실행하는 '''바이트코드(.class 파일)를 두는 메모리 영역'''이다.
//      - 바이트코드를 그대로 메모리에 두는 것이 아니라, '''멤버의 종류에 따라 적절하게 분류'''한다.
//    - 즉 '''클래스 코드가 이 영역에 놓이는 것'''이다.
//    - JVM은 코드를 실행할 때 '''이 영역에 놓은 명령어를 실행'''하는 것이다.
