...
> create table test1(
  c1 char(5), /* 한글 영어 상관없이 5자(고정 , 1글자만 들어가도 메모리는 5칸을 차지한다.)*/
  c2 varchar(5), /* 한글 영어 상관없이 5자(가변, 1글자가 들어가면 1글자만 메모리를 차지한다.)*/
  c3 varchar(21000)
  );
...
=> 숫자로 쓸 수는 있지만 2로 저장해도 저장이 되기 때문에
   숫자로 저장하는거 보다는 *true나 false로 저장하는 것을 추천*한다.
...
- 보통 일련번호를 저장할 정수 타입의 컬럼을 추가한다.
  - 예) 게시글 : 게시글 번호
- 대부분의 SNS 서비스들은 일련의 번호를 primary key 로 사용한다.
- 왜?
  - 회원 탈퇴의 경우,
    - 회원 탈퇴할 때 아이디도 제거한다.
...
- null을 따로 지정하지 않아도 기본값이 not null 로 설정된다.
  - 고로 PK는 무조건 지정해줘야 한다.
