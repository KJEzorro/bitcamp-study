...
// titleInput.style["display"] = "none"; // 아래 코드로 변경.
...
fetch("/todo/list")
  ...
  .then(function(todoList) {
    ...
    for (var todo of todoList) {
      ...
      <td><button type="button" class="btn btn-primary" onclick="updateTodo(${todo.no})">변경</button></td>
      <td><button type="button" class="btn btn-primary" onclick="deleteTodo(${todo.no})">삭제</button></td>`)
        .appendTo(tbody);
    }
    $("#x-todo-input").focus();
  });
...
titleInput.keyup(function(e) {
  ...
  if (e.keyCode == 27) { // ESC 키를 눌러 편집을 취소한다면
    cancelTodoEditing();
  } else if (e.keyCode == 13 && titleInput.val() != "" && originTitle != titleInput.val()) {
